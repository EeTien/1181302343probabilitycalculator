...
loandata = pd.read_csv(r'C:\Users\EeTien\Desktop\FYP\Presentable\credit_train.csv',index_col=[0])
...
def loan():
    st.write("""# Loan Approval Calculator""")
    st.sidebar.header('User Input Features')

    def user_input_features():
        CurrentLoanAmount = st.sidebar.number_input('Current Loan Amount', 0)
        Term = st.sidebar.slider('Term',0,1)
        CreditScore = st.sidebar.number_input('Credit Score', 0)
        AnnualIncome = st.sidebar.number_input('Annual Income', 0)
        Yearsincurrentjob = st.sidebar.slider('Years in current job', 0, 10)
        HomeOwnership = st.sidebar.slider('Home Ownership', 1, 3)
        MonthlyDebt = st.sidebar.number_input('Monthly Debt',0)

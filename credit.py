...
train_data = pd.read_csv(r'C:\Users\EeTien\Desktop\FYP\Presentable\cs-training.csv', index_col=[0])
...
def credit():
    ...
    def user_input_features():
        age = st.sidebar.slider('Age of applicant', 0)
        MonthlyIncome = st.sidebar.number_input('Monthly income', 1)
        DebtRatio = st.sidebar.number_input('Monthly expenses divided by monthly gross income', 0.000001)
        NumberOfDependents = st.sidebar.slider('Number of dependents in the family excluding the applicant ', 0, 20)
        ...
        NumberOfTime30to59DaysPastDueNotWorse = st.sidebar.slider('Number of times the applicant has been 30 to 59 days past due but not worse in the last 2 years', 0, 20)
        NumberOfTime60to89DaysPastDueNotWorse = st.sidebar.slider('Number of times the applicant has been 60 to 89 days past due but no worse in the last 2 years',0, 20)
        NumberOfTimes90DaysLate = st.sidebar.slider('Number of times the applicant has been 90 days or more past due', 0, 20)
